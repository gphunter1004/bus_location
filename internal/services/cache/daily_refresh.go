...
// PrintRefreshStatus 새로고침 상태를 로그로 출력 (디버깅용)
func (dcrm *DailyCacheRefreshManager) PrintRefreshStatus() {
	status := dcrm.GetRefreshStatus()
	windowInfo := dcrm.GetRefreshWindowInfo()

	dcrm.logger.Infof("🔄 새로고침 상태 V2:")
	dcrm.logger.Infof("   마지막 새로고침: %s", status["lastRefreshDate"])
	dcrm.logger.Infof("   현재 진행중: %t", status["isRefreshing"])
	dcrm.logger.Infof("   운영일자: %s", status["currentDate"])
	dcrm.logger.Infof("   다음 예상시간: %s", status["nextRefreshTime"])
	dcrm.logger.Infof("   새로고침 윈도우: %s ~ %s",
		windowInfo["refreshWindowStart"], windowInfo["refreshWindowEnd"])
	dcrm.logger.Infof("   윈도우 내 여부: %t", windowInfo["isInWindow"])
	dcrm.logger.Infof("   캐시 버전: %s", windowInfo["cacheVersion"])

	// V2 캐시 통계 출력
	if cacheStats, ok := status["cacheStats"].(map[string]interface{}); ok {
		dcrm.logger.Infof("   캐시 구조: %s", cacheStats["cache_type"])
		dcrm.logger.Infof("   Redis 활성화: %t", cacheStats["redis_enabled"])
	}
}

// RefreshSpecificRoute 특정 노선만 새로고침 (개별 관리용)
func (dcrm *DailyCacheRefreshManager) RefreshSpecificRoute(routeID string) error {
	dcrm.mutex.Lock()
	defer dcrm.mutex.Unlock()

	if dcrm.isRefreshing {
		return fmt.Errorf("전체 새로고침 진행 중입니다")
	}

	dcrm.logger.Infof("🔧 특정 노선 캐시 새로고침: %s", routeID)

	// 특정 노선만 새로고침
	if err := dcrm.redisCache.RefreshRouteCache(routeID); err != nil {
		dcrm.logger.Errorf("노선 %s 새로고침 실패: %v", routeID, err)
		return err
	}

	dcrm.logger.Infof("✅ 노선 %s 캐시 새로고침 완료", routeID)
	return nil
}
